/*
 * minimal_h_10stages.cpp — 前 10 個 stage（約 449 weak classifiers）
 *
 * 測試目的：從 G（5 stages, 136 wc）向上擴展，
 * 確認 10 stages / ~449 wc（完整版 2913 的 ~1/6）是否讓 clang reflow crash。
 *
 * 結構與 F/G 完全同形。唯一變數：外層迴圈上界 10。
 *
 * Stage weak classifier 數（累計 ~449）：
 *   stage 0:  9 wc   stage 5:  53 wc
 *   stage 1: 16 wc   stage 6:  64 wc
 *   stage 2: 27 wc   stage 7:  77 wc
 *   stage 3: 32 wc   stage 8:  85 wc
 *   stage 4: 52 wc   stage 9:  34 wc（實際數依 cascade 為準）
 *
